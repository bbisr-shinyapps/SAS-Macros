...
Last Update Date/Person/Contact: June 26, 2015/Yuan Liu/YLIU31@emory.edu
Current Version: V15
...
Notes: When using this macro to generate DATA DICTIONARY, set DICTIONARY=T , 
 The data dictionary contains variable name,label, formatted and unformatted levels for categorical variables, and descriptive statistics for all variables. 
The order of variables is the original order in the database. 

When using for variable description, the order of variables is the same as the input order, the order of variables
in the summary table is the same as the input order. For the best results, you may want to put the demographic 
variables together and also clinical characteristics variables together.     
...
%MACRO DESCRIPTIVE(DATASET=, CLIST=, NLIST=, CHI=F, TTEST=F, NONPAR=F, DICTIONARY=F, DOC=T, OUTPATH=, FNAME=, 
   FOOTNOTE=, DEBUG=F); 
...
PROC REPORT DATA=report_final HEADLINE HEADSKIP CENTER STYLE(REPORT)={JUST=CENTER} SPLIT='~' nowd 
      ...
      COLUMNS %if &dictionary = T %then %do; varnum var_name label fvalue  %end;
	          %else %do; order LABEL %end; levels  Measurement 
